...
function _getConfirmedResultKey(hostname, noHttps) {
    return "OfficeJSTrustKeyV2_" + (noHttps ? "http_" : "https_") + hostname;
}
...
;
try {
    var urlParams_1 = _getUrlParams();
    var e = Function._validateParams(urlParams_1, [{ name: "error", type: String, mayBeNull: false },
    ...
    if (!e) {
        var locale_1 = window.navigator.language;
        if (urlParams_1.validate) {
            function validateHostnameTrust(event) {
                ...
                if (!e) {
                    ...
                    if (!hostUserTrusted) {
                        _loadStrings(locale_1, function () {
            ...
            _loadStrings(locale_1, function () {
                if (SupportedError[urlParams_1.error])
                    _showInfoBar(urlParams_1.hostname, urlParams_1.noHttps, Strings.OfficeOM["L_" + urlParams_1.error]);
            });
        }
    }
}
