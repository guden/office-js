    ...
    (function (AddinNativeAction) {
        ...
        (function (Dialog) {
            ...
            function sendMessage(params) {
                if (windowInstance != null) {
                    var message = params.hostCallArgs[Microsoft.Office.WebExtension.Parameters.MessageContent];
                    var targetOrigin = params.hostCallArgs[Microsoft.Office.WebExtension.Parameters.TargetOrigin] || null;
                ...
                function receiveCloseDialogMessage(event) {
                    ...
                    if (event.source == window.opener &&
                        ...
                        if (typeof event.data === "string" && event.data.indexOf(closeDialogKey) > -1) {
                            ...
                            var message = {};
                            message[OSF.DDA.PropertyDescriptors.MessageContent] = event.data;
...
OSF.DDA.WAC.Delegate.openDialog = function OSF_DDA_WAC_Delegate$OpenDialog(args) {
    ...
    function includeTaskpaneUrlAtTheEnd(taskpaneUrl) {
        var appDomains = OSF._OfficeAppFactory.getInitializationHelper()._appContext._appDomains;
        var parsedTaskpaneUrl = OSF.OUtil.parseUrl(taskpaneUrl);
        if (!parsedTaskpaneUrl.protocol || !parsedTaskpaneUrl.hostname || parsedTaskpaneUrl.port === undefined) {
            showDialogCallback(OSF.DDA.ErrorCodeManager.errorCodes.ooeRequireHTTPS);
            return false;
        }
        if (!appDomains || appDomains.length === 0) {
            OSF._OfficeAppFactory.getInitializationHelper()._appContext._appDomains = [taskpaneUrl];
        }
        else {
            var isFound = false;
            var i;
            for (i = appDomains.length - 1; i >= 0; i--) {
                var parsedDomainUrl = OSF.OUtil.parseUrl(appDomains[i]);
                if (urlCompare(parsedTaskpaneUrl, parsedDomainUrl)) {
                    if (i < appDomains.length - 1) {
                        OSF._OfficeAppFactory.getInitializationHelper()._appContext._appDomains.push(OSF._OfficeAppFactory.getInitializationHelper()._appContext._appDomains.splice(i, 1)[0]);
                    }
                    isFound = true;
                    break;
                }
            ...
            if (!isFound) {
                OSF._OfficeAppFactory.getInitializationHelper()._appContext._appDomains.push(taskpaneUrl);
            }
        }
        return true;
    }
    ...
    if (!includeTaskpaneUrlAtTheEnd(taskpaneUrl)) {
        return;
    }
...
OSF.DDA.WAC.Delegate.messageParent = function OSF_DDA_WAC_Delegate$MessageParent(args) {
    ...
    if (window.opener != null) {
        if (!hasTargetOrigin) {
            var appDomains = OSF._OfficeAppFactory.getInitializationHelper()._appContext._appDomains;
            if (!appDomains || appDomains.length === 0) {
                var errorMessage = "taskpaneUrl not found";
                throw new Error(errorMessage);
            }
            var taskpaneUrl = appDomains[appDomains.length - 1];
            if (!OfficeExt.AddinNativeAction.Dialog.validateTaskpaneDomain(taskpaneUrl, allowSubDomains)) {
